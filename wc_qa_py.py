...
def word_count_per_doc(document, F=-1):
		...
		for word in document.split(' '):
			# word è usato come scorritore nel ciclo for e la funzione split
			# dovrebbe separare i vari elementi del documento di uno spazio
...
def word_count_1(docs):
	...
	# la flatMap function prende il documento e genera una lista di pairs
	# la funzione word_count_per_doc che conteggia le local frequencies, ovvero
	#conta il numero di parole uguali per ogni riga, difatti ogni riga è un documento (da quanto ricordo)
